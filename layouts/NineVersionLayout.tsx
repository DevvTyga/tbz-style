...
import { NINE_ITEMS, TBZ } from "../shared/data/constants";
...
export default function NineVersionLayout() {
  ...
  const [tbzMemberStateMap, setTbzMemberStateMap] = useState(
  ...
  return (
    <div>
          ...
          >
            {NINE_ITEMS.map((item: string, index: number) => (
              <SelectButton
                title={item}
                index={index}
                setShowModal={() => setShowModal(item, setTbzMemberStateMap)}
                memberImg={tbzMemberStateMap[item].memberImg}
                memberName={tbzMemberStateMap[item].memberName}
              />
            ))}
            <div className="p-2">
      ...
      {NINE_ITEMS.map((item: string) =>
        tbzMemberStateMap[item].showModal ? (
          <CustomModal
            members={TBZ}
            title={item}
            setStateMap={setTbzMemberStateMap}
            memberType={item}
          />
        ) : null
      )}
